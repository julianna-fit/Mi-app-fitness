const versiculosDelAno = [
  {
    texto: pruebaaaa"Todo lo puedo en Aquel que me fortalece.",
    referencia: "Filipenses 4:13",
    reflexion: "Dios te da la fuerza incluso cuando sientes que no puedes más.",
    mensaje: "Hoy eres más fuerte de lo que crees 💜"
  },
  {
    texto: "Jehová es mi pastor; nada me faltará.",
    referencia: "Salmos 23:1",
    reflexion: "Dios cuida cada detalle de tu vida.",
    mensaje: "Confía, Dios ya tiene el control 🙏"
  },
  {
    texto: "Esfuérzate y sé valiente.",
    referencia: "Josué 1:9",
    reflexion: "No estás sola, Dios va contigo.",
    mensaje: "Hoy camina con valentía 💪"
  },
  {
    texto: "Los que esperan en Jehová tendrán nuevas fuerzas.",
    referencia: "Isaías 40:31",
    reflexion: "Esperar en Dios no es perder tiempo, es recargar el alma.",
    mensaje: "Dios renovará tus fuerzas hoy ✨"
  },
  {
    texto: "Echa sobre Jehová tu carga, y Él te sustentará.",
    referencia: "Salmos 55:22",
    reflexion: "No cargues sola lo que Dios quiere ayudarte a llevar.",
    mensaje: "Suelta la carga y respira en fe 🌷"
  },
  {
    texto: "Mi gracia es suficiente para ti.",
    referencia: "2 Corintios 12:9",
    reflexion: "Aunque te sientas débil, la gracia de Dios sigue siendo suficiente.",
    mensaje: "No necesitas hacerlo todo sola 💜"
  },
  {
    texto: "Este es el día que hizo Jehová; nos gozaremos y alegraremos en él.",
    referencia: "Salmos 118:24",
    reflexion: "Hoy también es regalo de Dios. Vívelo con gratitud.",
    mensaje: "Hoy vale la pena sonreír ☀️"
  }
];
